...
func GitHubActivityHandler(w http.ResponseWriter, r *http.Request) {
	username := os.Getenv("GITHUB_USERNAME")
	if username == "" { username = "daikiito-dk" }
	if strings.TrimSpace(username) == "" { httpx.Error(w, http.StatusBadRequest, "github username is required"); return }

	req, err := http.NewRequestWithContext(r.Context(), http.MethodGet, "https://api.github.com/users/"+username+"/events/public?per_page=20", nil)
	...
	for _, event := range events {
		...
		activities = append(activities, GitHubActivity{ID: event.ID, Type: event.Type, Repo: event.Repo.Name, Summary: summary, CreatedAt: created})
	}
