...
func NewGitHubActivityHandler(repo *repository.GitHubActivityRepository) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		for _, event := range events {
			...
			case "DeleteEvent": summary = "deleted a repository reference"
			case "WatchEvent": summary = "starred a repository"
			case "ForkEvent": summary = "forked a repository"
			}
			if err := repo.Upsert(r.Context(), event.ID, event.Type, event.Repo.Name, summary, "", created); err != nil { httpx.Error(w, http.StatusInternalServerError, "could not save GitHub activity"); return }
		}
		rows, err := repo.List(r.Context(), 100); if err != nil { httpx.Error(w, http.StatusInternalServerError, "could not load GitHub activity"); return }
		activities := make([]GitHubActivity, 0, len(rows)); for _, row := range rows { activities = append(activities, GitHubActivity{ID: row.ID, Type: row.Type, Repo: row.Repo, Summary: row.Summary, URL: row.URL, CreatedAt: row.CreatedAt}) }
