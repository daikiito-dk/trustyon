...
function App() {
  ...
  const addTask = async (event: FormEvent) => {
    ...
    setSaving(true)
    try {
      const task = await getJSON<Task>('/api/tasks', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title, status: 'todo', priority }) })
      setTasks((current) => [task, ...current])
      setNewTask('')
      setHealth('online')
    } catch { setHealth('offline') } finally { setSaving(false) }
  }
  ...
  const addProject = async (event: FormEvent) => {
    ...
    try {
      ...
      const project = await getJSON<Project>('/api/projects', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, slug, status: 'active' }) })
  ...
  const addNote = async (event: FormEvent) => {
    ...
    setSaving(true)
    try {
      const note = await getJSON<Note>('/api/notes', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title, body: '' }) })
  ...
  const toggleTask = async (task: Task) => {
    ...
    try {
      const updated = await getJSON<Task>(`/api/tasks/${task.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...task, status }) })
  ...
  return (
    <main className="shell">
      <header className="header">
        <div><span className="eyebrow">PERSONAL DEVELOPER OS</span><h1>Trustyon</h1></div>
        <div className={`status ${health}`}><span /> API {health}</div>
      </header>

      <section className="hero">
        <p className="eyebrow">TODAY</p><h2>Build. Learn. Ship.</h2>
      ...
      <section className="grid">
        <article className="card wide">
          ...
          <form className="task-form" onSubmit={addTask}>
            ...
            <button type="submit" disabled={saving || !newTask.trim()}>Add task</button>
          </form>
        ...
        <article className="card">
          ...
          <form className="mini-form" onSubmit={addProject}><input value={newProject} onChange={(event) => setNewProject(event.target.value)} placeholder="New project" aria-label="New project" /><button type="submit" disabled={saving || !newProject.trim()}>+</button></form>
          {projects.length > 0 ? <ul className="simple-list">{projects.slice(0, 5).map((project) => <li key={project.id}><span>●</span>{project.name}</li>)}</ul> : <p className="muted">{loading ? 'Loading projects…' : 'No projects yet.'}</p>}
        </article>

        <article className="card">
          <div className="card-title"><span>Notes</span><strong>{loading ? '—' : notes.length}</strong></div>
          <form className="mini-form" onSubmit={addNote}><input value={newNote} onChange={(event) => setNewNote(event.target.value)} placeholder="New note" aria-label="New note" /><button type="submit" disabled={saving || !newNote.trim()}>+</button></form>
          {notes.length > 0 ? <ul className="simple-list">{notes.slice(0, 5).map((note) => <li key={note.id}><span>↳</span>{note.title}</li>)}</ul> : <p className="muted">{loading ? 'Loading notes…' : 'Capture ideas and technical notes.'}</p>}
        </article>
      </section>
    </main>
  )
